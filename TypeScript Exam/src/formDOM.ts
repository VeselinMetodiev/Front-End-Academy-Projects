...
export function loggedInStateDOM(user: User) {
  ...
  if(user.userRole === UserRole.USER){
    formHtml = `<h1>Congrats ${user.firstName} - ${user.userRole} You successfully logged in!</h1>
    <div class="container">
          <label for="firstName"><b>First Name</b></label>
          <input type="text" placeholder="Enter first name" name="firstName" required>
          <span id="firstNameError" class="helper-text" data-error="wrong" data-success="right"></span>
    ...
    formHtml = `<h1>Admin Screen</h1> <br>
    <h2>Congrats ${user.firstName} - ${user.userRole} You successfully logged in!</h2>`
  }
  return formHtml;
  }