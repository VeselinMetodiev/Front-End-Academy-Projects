...
import { IdType, QuestionListener, Optional, QuestionTypes } from '../model/shared-types'
...
interface QuestionsFormState {
  ...
  type: QuestionTypes | undefined,
  id: string,

}

export default class QuestionForm extends Component<QuestionsFormProps, QuestionsFormState> {
  ...
  type: this.props.question?.type || undefined,
  ...
  handleQuestionReset = () => {
    this.setState({text: '', pointsNumber: 0, imageURI: '', type: undefined, answers: []})
}

handleQuestionSubmit = (values : Partial<Question>) => {
  this.props.onCreateQuestion(new Question(
      values.text!,
      values.pointsNumber!,
      this.state.answers,
        ...
        {({ values, handleChange, errors, setFieldTouched, touched, isValid, handleSubmit, resetForm }) => (
          <View style={styles.registrationForm}>
            ...
            {touched.text && errors.text &&
              <Text style={{ fontSize: 12, color: '#FF0D10' }}>{errors.text}</Text>
            }            
            <TextInput
            ...
            {touched.pointsNumber && errors.pointsNumber &&
              <Text style={{ fontSize: 12, color: '#FF0D10' }}>{errors.pointsNumber}</Text>
            }
             <TextInput
            ...
            <FontAwesome.Button
              color="#841584"
              disabled={!isValid}
              onPress={() => {this.handleQuestionSubmit(values); resetForm()}}
            ...
            Submit
              ...
              <FontAwesome.Button
          onPress={() => {this.handleQuestionReset; resetForm()}}
          color="#842317"
